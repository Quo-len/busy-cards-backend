...
module.exports = {
	getAllFavorites: async (req, res) => {
		try {
			const favorites = await Favorite.find({ user: req.query.userId }).populate('mindmap').select('-__v');

			const result = favorites.map((f) => ({
				_id: f._id,
				mindmap: f.mindmap.title,
				user: f.mindmap.title,
			}));
	...
	getFavorite: async (req, res) => {
		try {
			const favorite = await User.findById(req.params.id);
